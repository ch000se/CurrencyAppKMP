...
import org.example.currencyapp.util.DoubleConverter
import org.example.currencyapp.util.GetBebasFontFamily
import org.example.currencyapp.util.calculateExchangeRate
import org.example.currencyapp.util.convert
        ...
        Column(
            ...
            AnimatedVisibility(
                ...
                Column {
                    Text(
                        modifier = Modifier.fillMaxWidth(),
                        text = "1 ${source.getSuccessData().code} = ${target.getSuccessData().value / source.getSuccessData().value} ${target.getSuccessData().code}",
                        fontSize = MaterialTheme.typography.bodySmall.fontSize,
                        color = if (isSystemInDarkTheme()) Color.White.copy(0.5f)
                    ...
                    Text(
                        modifier = Modifier.fillMaxWidth(),
                        text = "1 ${target.getSuccessData().code} = ${source.getSuccessData().value / target.getSuccessData().value} ${source.getSuccessData().code}",
                        fontSize = MaterialTheme.typography.bodySmall.fontSize,
                        color = if (isSystemInDarkTheme()) Color.White.copy(0.5f)
                        else Color.Black.copy(0.5f),
                        textAlign = TextAlign.Center,
